...
$checkColumn = $connection->query("SHOW COLUMNS FROM `test_chet_view` LIKE 'std_dev'");
if ($checkColumn->num_rows == 0) {
    $connection->query("ALTER TABLE `test_chet_view` ADD COLUMN `std_dev` FLOAT");
}

$sql = "SELECT `avg_time`, `accuracy`, `misses`, `std_dev`, `date` FROM `test_chet_view` WHERE `user_id` = ?";
