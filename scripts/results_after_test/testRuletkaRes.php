...
$checkColumn = $connection->query("SHOW COLUMNS FROM `test_ruletka` LIKE 'std_dev'");
if ($checkColumn->num_rows == 0) {
    $connection->query("ALTER TABLE `test_ruletka` ADD COLUMN `std_dev` FLOAT");
}

$sql = "SELECT `avg_time`, `accuracy`, `misses`, `std_dev`, `date` FROM `test_ruletka` WHERE `user_id` = ?";
